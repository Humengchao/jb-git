...
/**
 * A plan row that is not a commit: `exec` runs a shell command at that point,
 * `break` pauses the rebase there — Git's todo lines of the same names. Both
 * run unattended through Git's own machinery, and a pause is owned by the same
 * Continue/Abort handling an `edit` stop uses.
 */
export type RebaseInterjection =
  | { readonly kind: "exec"; readonly command: string }
  | { readonly kind: "break" };

/** A plan is commit steps interleaved with interjections, in replay order. */
export type PlanRow = RebaseStep | RebaseInterjection;

export function isRebaseInterjection(row: PlanRow): row is RebaseInterjection {
  return "kind" in row;
}
...
/** Bound on an `exec` row's command, which lands verbatim on a todo line. */
export const EXEC_COMMAND_LIMIT = 4096;
 ...
 *
 * Interjections are transparent to the commit rules: fold legality is decided
 * by the commit rows alone, matching Git, where a `fixup` after an `exec` or
 * `break` still folds into the previous kept commit.
 */
export function validateRebasePlan(rows: readonly PlanRow[]): string | undefined {
  if (rows.length === 0) return "Select at least one commit to rebase.";

  const steps = rows.filter((row): row is RebaseStep => !isRebaseInterjection(row));
  ...
  for (const row of rows) {
    if (isRebaseInterjection(row)) {
      if (row.kind === "exec") {
        // The command lands on one todo line, so a newline would inject
        // another instruction Git would try to parse.
        if (!row.command.trim()) return "An exec row needs a command.";
        if (row.command.length > EXEC_COMMAND_LIMIT || /[\r\n\0]/.test(row.command)) {
          return "An exec command must be a single line.";
        }
      }
      continue;
    }
...
export function buildRebaseTodo(
  ...
  const runs = messageRuns(rows.filter((row): row is RebaseStep => !isRebaseInterjection(row)));
  ...
  for (const row of rows) {
    if (isRebaseInterjection(row)) {
      // Git runs an exec through its own shell and stops the sequence on a
      // non-zero exit; a break parks the rebase the way an edit stop does.
      lines.push(row.kind === "exec" ? `exec ${row.command}` : "break");
      continue;
    }
    const step = row;
 ...
 * so the original order has to be compared rather than the actions alone. An
 * interjection makes the plan worth running even with history untouched: an
 * exec row is there to run, a break row to pause.
 */
export function isNoOpPlan(rows: readonly PlanRow[], originalOrder: readonly string[]): boolean {
  const steps = rows.filter((row): row is RebaseStep => !isRebaseInterjection(row));
