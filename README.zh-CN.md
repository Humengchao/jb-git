...
| 带本地修改的分支切换 | 已实现 | Smart Checkout 把已跟踪、已暂存和未跟踪修改保存到临时 Stash，以不可变 OID 定位，切换后恢复 Working Tree 与 Index。切换或恢复失败时 Stash 会保留供恢复。分支弹窗具备 IDEA 的分组：**最近**列出近期检出过的分支（从 HEAD 的 reflog 读取，一次检出的两端都算），**收藏**列出用 ★ 按钮加星的分支——按工作区和仓库保存，分支删除后自动遗忘；每个有活跃上游的本地分支都带一个**更新**按钮，无需检出即可从上游快进（当前分支则执行 pull）。 |
...
2. 点击扩展视图右上角的 `...` 菜单。
3. 选择“从 VSIX 安装…”。
4. 选择项目生成的 `jb-git-0.1.25.vsix`。
5. 根据提示重新加载 VS Code。

也可以通过命令行安装：

```bash
code --install-extension jb-git-0.1.25.vsix
```
