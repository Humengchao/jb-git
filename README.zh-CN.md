...
| HEAD / Index / Working Tree 审阅 | 已实现 | Local Changes 明确展示两层差异：`HEAD → Index`（已暂存）和 `Index → Working Tree`（未暂存），均支持文件 Diff；文本文件还支持 Hunk 级暂存/取消暂存，并在写入 Index 前验证 Hunk 没有过期。工作区侧的 hunk 还带有 IDEA 的**回滚**：该处改动回到 Index 的内容，文件内其他改动和已暂存内容保持不变，与其他回滚一样需要确认，并在“搁置”中保留一份**只包含被丢弃那处改动**的恢复条目——它是记录而非搁置，因为搁置会把文件的改动整体移出工作区，那样会连带丢掉本该保留的 hunk。较长的更改列表只绘制前 500 行，其余一键展开：该面板每次刷新都会重建，几千个变更文件每次要付出数万个元素的代价。**创建补丁…**会把勾选的更改（与提交取用的选择一致，包含已暂存内容）保存为 `-p1` 补丁，`git apply` 与 IDEA 自身的 Apply Patch 都能应用。 |
...
| Changelist | 部分实现 | 支持创建、重命名/描述、设为 Active、删除、整文件移动、重命名路径迁移和指定 Changelist 提交。**同一文件内的不同改动**现在可以像 IDEA 那样归属到不同 Changelist：展开文件会按 HEAD 列出它的各处改动及所属列表，`Move...` 可以改归属；当两处编辑落在同一个 hunk 里时，还可以逐行勾选后用 `Move Lines...` 移动——行级认领比所在 hunk 的认领更具体，优先生效。提交某个 Changelist 时只取属于它的部分——文件所属列表提交别人没认领的全部（含之后新增的改动），认领方只提交自己认领的 hunk 或行，其余留在工作区。改动是按其内容而不是行号记住的，所以周围行移动时归属不丢、跟随文件改名迁移、只有改动本身被撤销时归属才失效。以完整内容提交时，如果会把别的 Changelist 的工作一并带走，会先明确提示。任务上下文和 Changelist 冲突策略仍是差距。 |
| Rollback、Shelf 与未跟踪文件删除 | 已实现 | 搁置页签与 IDEA 一致：条目可展开查看包含的文件，菜单提供**取消搁置**（恢复并移除条目，与 IDEA 相同）、**取消搁置并保留**、**取消搁置到更改列表…**、**显示差异**、**重命名…**和**删除…**；`JB Git: Open Shelf` 可直接跳到该页签。分支已经前进、补丁不再适用的搁置不会再以 Git 的 "patch does not apply" 失败：它会通过三路合并恢复，作为冲突交给合并编辑器处理，并在解决之前保留该条目。回滚已跟踪文件前会先在 Shelf 保留 recovery entry；未跟踪文件通过系统 Trash 移除，不做不可恢复删除；冲突文件不允许单独回滚，以免静默丢掉一侧内容。未跟踪文件的**忽略…**与 IDEA 一致：可选择精确文件、所在目录或同扩展名的所有文件，写入仓库的 `.gitignore` 或仅本地克隆生效的 `.git/info/exclude`，规则会加锚点并转义，只命中所指的对象，且重复操作不会重复追加。 |
...
| Interactive Rebase 编辑器 | 已实现 | `JB Git: Interactively Rebase from Commit` 会打开可视化序列编辑器（界面已本地化），支持拖拽手柄或 Alt+↑/↓ 重排以及 `pick`、`reword`、`edit`、`squash`、`fixup`、`drop`。`edit` 即 IDEA 的“停下修改”：变基应用该提交后停住（此时 Git 退出码为 0，因此扩展读取 sequencer 状态而不是相信退出码），提示你修改或测试后 Continue，暂存的本地更改也会保持暂存而不是恢复到正在修改的提交上。序列里还可以插入 `exec` 行（在该位置用 Git 自己的 shell 运行一条命令，非零退出会使序列停在那里）和 `break` 行（在该位置暂停，Continue 继续）——它们像 `edit` 停点一样由同一套 Continue/Abort 机制接管。向“停下修改”的提交所在的组里 squash 会被拒绝——squash 的改写以组长的主题为守卫，而 edit 停点恰恰会改掉这个主题。改写消息的动作会降级为 Git 可无人值守执行的 todo，因此不会启动任何编辑器；冲突暂停后 `Continue` 仍会正确应用 reword 的消息。工作区有改动时会像 IDEA 一样提供 stash：只有真正开始 rebase 时才会暂存，结束后连同 Index 一起恢复；如果 rebase 因冲突停下，改动会保留在 stash 里，而不是叠加到未解决的冲突上。Git 自己的 `rebase.autoStash` 仍然关闭，因为它会无条件恢复。未跟踪文件不再算作阻塞——Git 本来就能在它们之上重放。相比 IDEA 更窄：起始提交必须是 HEAD 的祖先，范围内含 merge 时直接拒绝而不是压平。 |
| File History 与 Blame | 部分实现 | File History 和命令输出式 Blame 已实现，并能安全处理特殊路径。文件上的**与分支比较…**和**与修订比较…**与 IDEA 一致：另一分支/标签上的版本，或从该文件自身（跨重命名）历史中选出的某个修订，会在原生差异视图中与工作区版本对照打开。File History 会像 IDEA 一样跨重命名追踪文件（`--follow`，Git 只允许用于单个字面路径，因此 Log 里手动输入的后缀筛选仍是普通路径筛选），其详情面板也按 IDEA 的按修订文件列表只显示该文件——以 Git 自己的 `--follow` 遍历回答每个提交里它叫什么，因此改名前的旧名字能正确保留，而撞名的无关文件绝不会被误认。**显示所选内容的历史**即 IDEA 的 Show History for Selection：在编辑器里选中若干行，Log 就收窄为改动过这些行的提交（跨重命名），路径筛选旁的 `Lines` 芯片可一键回到整个文件的历史。选区会先通过与 `HEAD` 的差异映射到已提交的文件上——替换掉旧行的新行继承旧行的历史，纯新增的选区会被告知尚无历史，未保存的缓冲区会先提示保存，因为磁盘上的差异看不到它。 |
...
2. 点击扩展视图右上角的 `...` 菜单。
3. 选择“从 VSIX 安装…”。
4. 选择项目生成的 `jb-git-0.1.36.vsix`。
5. 根据提示重新加载 VS Code。

也可以通过命令行安装：

```bash
code --install-extension jb-git-0.1.36.vsix
```
...
接下来的主要差距是完整 Base-aware 三方 Merge、跨仓库事务回滚、持久化大历史索引，以及 SSH、WSL、Dev Container、远程 Extension Host、无障碍和不同 Git 版本的系统性验证。
