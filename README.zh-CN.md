...
- 支持上一处/下一处更改（F7 / Shift+F7）、逐块接受左侧/两侧/右侧、整文件接受左侧或右侧，以及拖动调整三栏宽度；`Esc` 中止。
...
2. 点击扩展视图右上角的 `...` 菜单。
3. 选择“从 VSIX 安装…”。
4. 选择项目生成的 `jb-git-0.1.19.vsix`。
5. 根据提示重新加载 VS Code。

也可以通过命令行安装：

```bash
code --install-extension jb-git-0.1.19.vsix
```
