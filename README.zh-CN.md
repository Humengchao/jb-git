...
| --- | --- | --- |
| Git 工具窗口与 Log | 部分实现 | 一个底部 `Git` Panel 提供 Log、Console、Local Changes 和 Shelf。提交图、筛选、分支比较、每次增量加载 300 条和大列表虚拟化已实现；持久化历史索引及超大仓库的完整搜索仍有差距。每个本地分支带 IDEA 的传入/传出标记——↓n 已抓取未合并、↑n 尚未推送，右对齐显示；上游被删除的分支显示 `gone` 而不是零。搜索框像 IDEA 一样分两层：输入即时筛选已加载窗口，回车把文本交给 Git 在全部历史中搜索提交消息（字面匹配、不分大小写，走与普通日志相同的读取路径，因此提交图、选中和虚拟化仍是同一套实现）；输入十六进制则跳转到该提交——即使它在已加载窗口之外，日志会以它为根重新加载。提交列表支持 IDEA 式多选——Ctrl/Cmd+点击切换、Shift+点击或 Shift+方向键扩展范围——详情栏会汇总所选提交；右键所选内容提供 **Cherry-Pick Selected**（按历史顺序逐个拣选，遇到冲突即停并说明完成到第几个，其余提交留给你重做）和恰好选中两个时的 **Compare Versions**（从旧到新的只读 diff）。宿主不信任 Webview 发来的点击顺序：以日志自身的顺序为准。所选提交还能像 IDEA 一样直接改写历史：**Drop Commit** 删除所选提交，**Squash Selected** 把它们聚拢到最旧的那个并合并全部消息，两者都降级为与序列编辑器相同的无人值守变基计划——同样的 stash 询问、同样的冲突停点、同样的拒绝条件（范围含 merge、选择不在线性历史上）；压缩不相邻的提交时，确认框会说明中间的提交将被重排。 |
...
2. 点击扩展视图右上角的 `...` 菜单。
3. 选择“从 VSIX 安装…”。
4. 选择项目生成的 `jb-git-0.1.24.vsix`。
5. 根据提示重新加载 VS Code。

也可以通过命令行安装：

```bash
code --install-extension jb-git-0.1.24.vsix
```
