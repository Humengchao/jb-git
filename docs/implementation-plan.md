...
1. Add IDEA's Compare contents modes to the merge editor — Base/Result and Left/Right at minimum — now that each conflict carries its base.
2. Add movement and copy detection (`-M`/`-C`) plus whitespace options to editor-gutter Blame, and let the gutter annotate an arbitrary chosen revision.
